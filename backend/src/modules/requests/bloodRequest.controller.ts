...
export class BloodRequestController {
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await BloodRequestService.updateRequest(req.params.id, req.user.userId, parsedData.data);
  ...
  static async cancel(req: Request, res: Response, next: NextFunction) {
    try {
      if (!req.user) throw new AppError(401, 'UNAUTHENTICATED', 'Missing user');
      const result = await BloodRequestService.cancelRequest(req.params.id, req.user.userId);
      res.status(200).json(SuccessResponse(result, req.id));
    } catch (error) { next(error); }
  }
  ...
  static async verify(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await BloodRequestService.verifyRequest(req.params.id, req.user.userId, activate);
      res.status(200).json(SuccessResponse(result, req.id));
    } catch (error) { next(error); }
  }
  ...
  static async fulfill(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await BloodRequestService.fulfillUnits(req.params.id, units, req.user.userId);
      res.status(200).json(SuccessResponse(result, req.id));
    } catch (error) { next(error); }
  }
}
